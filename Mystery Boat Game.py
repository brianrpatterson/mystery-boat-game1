...
terry = Person('Terry', 'Shame what happened to Kevin. Can\'t say I\'ll miss his dessert hoarding though.', 'I\'m on my way to a pest control convention. I work as a sales rep for a chemical company.',
               'After I went to bed last night I heard an awful rukus coming from the deck. You weren\'t attacked were you?')
miguel = Person('Miguel', 'Crazy to think. We knew Kevin for such little time and now all we have to remember him by is the memory of his idiocy.',
                'When we get to land I will probably get some more red paint to decorate this bird.', 'I\'m not sure anyone else noticed but when we found Kevin his hair was soaking wet.')
jack = Person('Jack', 'It\'s so exciting! A real murder! In person! This is great research for my book!',
              'I am writing a murder mystery that takes place in a medieval castle. The killer is going to commit the crime with a candlestick.', 'I was in my room all night except for when you asked me for a towel.')

kevinroom = Room('Kevin\'s Cabin',
                 'In the room there is little more than a portrait of the most beautiful woman you have ever seen.')
terryroom = Room('Terry\'s Cabin',
                 'Terry is lounging on the bed and you observe an unopened container of rat poison, a lab coat, and rubber gloves.', terry)
miguelroom = Room('Miguel\'s Cabin',
                  'Miguel is whittling a small bird. His room contains a several knives, blocks of wood, and red stained cutting gloves.', miguel)
...
franklinroom = Room('Franklin\'s Cabin',
                    'Your own room. There is an empty wallet, some books, and an address book on the desk.')
salon = Room(
    'The Salon', 'On the table there are playing cards from an unfinished game and several empty beer bottles.')
galley = Room(
    'The Galley', 'A small, fairly plain kitchen. Nothing seems out of place apart from some wet footprints')
head = Room(
    'The Head', 'A cramped ship\'s bathroom. There is a lot of water all over the sink and floor.')
...
print('Welcome!')
print('''You(Francis) are a passenger on a boat bound for another country.
      Things were going well with you and your fellow travelers until one of them, Kevin, was found dead. You have been designated as the detective. Your job is to interview the other passengers, inspect the ship, and determine who the murderer is.''')
